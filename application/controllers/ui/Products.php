...
class Products extends Base
{
    ...
    public function manage_timetable()
    {
        ...
        $product = $this->db->get_where("products", compact("id"))
                            ->row(0, "array");

        $this->view('products/timetable', compact("id", "product"));
    }

    public function delete_service_event()
    {
        $service_date_from = $this->input->post("service_date_from2", true);
        $product_id        = $this->input->post("product_id2");

        $res = $this->db->where("date", $service_date_from)
                        ->where("product_id", $product_id)
                        ->delete("service_dates");
        redirect("ui/products/manage_timetable?id=" . $product_id);
    }

    public function add_service_event()
    {
        $from_time         = $this->input->post("from_time", true);
        $to_time           = $this->input->post("to_time", true);
        $service_date_from = $this->input->post("service_date_from", true);
        $service_date_to   = $this->input->post("service_date_to", true);
        $is_range_date     = $this->input->post("is_range_date", true);
        $product_id        = $this->input->post("product_id", true);

        $capacity = $this->input->post("capacity",true);

        if (!empty($from_time) && !empty($to_time) && !empty($service_date_from)) {
            if ($is_range_date == 'yes') {

                $begin    = new DateTime($service_date_from);
                $end      = new DateTime($service_date_to);
                $interval = DateInterval::createFromDateString('1 day');
                $period   = new DatePeriod($begin, $interval, $end);
                foreach ($period as $dt) {
                    $this->add_service_date(array(
    ...
    private function add_service_date($data)
    {
        $this->db->where("date", $data['date'])
                 ->where("product_id", $data['product_id'])
                 ->delete("service_dates");
        $this->db->insert("service_dates", $data);
    }

    public function get_events()
    {
        $product_id = $this->input->get("id", true);
        $res        = $this->db->get_where("service_dates", array("product_id" => $product_id))->result_array();
        $ret        = array();
        foreach ($res as &$r) {
            $ret[] = array(
                "id"    => $r['id'],
                "title" => $r['from_time'] . '--' . $r['to_time'],
                "start" => $r['date'] . " " . $r['from_time'],
                "end"   => $r['date'] . " " . $r['to_time']
            );
        }
        echo json_encode(array("events" => array_values($ret)));
    }
}
